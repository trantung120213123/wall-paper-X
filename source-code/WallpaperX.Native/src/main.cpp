...
constexpr UINT IDM_TRAY_OPEN_EDITOR = 2001;
...
void ShowControllerWindow() {
    ...
    ShowWindow(g_app.controller, SW_SHOW);
    ShowWindow(g_app.controller, SW_RESTORE);
    SetForegroundWindow(g_app.controller);
}

void HideControllerWindow() {
    if (!g_app.controller) {
        return;
    }
    ShowWindow(g_app.controller, SW_HIDE);
}
...
void AddTrayIcon() {
    ...
    g_app.trayIcon.hWnd = g_app.controller;
    g_app.trayIcon.uID = 1;
    g_app.trayIcon.uFlags = NIF_MESSAGE | NIF_ICON | NIF_TIP;
    g_app.trayIcon.uCallbackMessage = WM_TRAYICON;
    HICON icon = LoadIconW(g_app.instance, MAKEINTRESOURCEW(IDI_APP_ICON));
    if (!icon) {
        icon = LoadIconW(nullptr, IDI_APPLICATION);
    }
    g_app.trayIcon.hIcon = icon;
    wcscpy_s(g_app.trayIcon.szTip, L"WallpaperX Native");

    g_app.trayAdded = Shell_NotifyIconW(NIM_ADD, &g_app.trayIcon) == TRUE;
}
...
void ExitApplication() {
    ...
    if (g_app.wallpaper) {
        DestroyWindow(g_app.wallpaper);
        g_app.wallpaper = nullptr;
    }

    if (g_app.controller) {
        DestroyWindow(g_app.controller);
        g_app.controller = nullptr;
    }
}
...
void ShowTrayMenu() {
    ...
    AppendMenuW(menu, MF_STRING, IDM_TRAY_OPEN_EDITOR, L"Open Editor");
    AppendMenuW(menu, MF_STRING, IDM_TRAY_EXIT, L"Exit Wallpaper");

    POINT pt {};
    GetCursorPos(&pt);
    SetForegroundWindow(g_app.controller);
    const UINT command = TrackPopupMenu(menu, TPM_RETURNCMD | TPM_NONOTIFY, pt.x, pt.y, 0, g_app.controller, nullptr);
    DestroyMenu(menu);

    if (command == IDM_TRAY_OPEN_EDITOR) {
...
std::wstring GetInstalledBinaryPath(const std::wstring& fileName) {
    const auto programFiles = GetKnownFolderPath(FOLDERID_ProgramFiles);
    if (!programFiles.empty()) {
        const auto installed = fs::path(programFiles) / kInstallFolderName / fileName;
...
std::wstring GetRegisteredEditorPath() {
    const auto installed = GetInstalledBinaryPath(kEditorExeName);
    if (!installed.empty()) {
        return installed;
    }
...
std::wstring GetRegisteredStartupPath() {
    const auto installed = GetInstalledBinaryPath(kStartupExeName);
    if (!installed.empty()) {
        return installed;
    }
    return L"";
...
void SetAutoStart(bool enabled) {
    ...
    if (enabled) {
        auto startupExe = GetRegisteredStartupPath();
        if (startupExe.empty()) {
            startupExe = GetRegisteredEditorPath();
        }
        std::wstring value = L"\"" + startupExe + L"\"";
        if (fs::path(startupExe).filename().wstring() == kEditorExeName) {
            value += L" --background";
        }
...
void RegisterVideoContextMenu() {
    const auto exePath = GetRegisteredEditorPath();
...
bool AreFilesIdentical(const fs::path& a, const fs::path& b) {
    std::error_code ec;
    ...
    std::vector<char> left(kBufferSize);
    std::vector<char> right(kBufferSize);

    while (first && second) {
        first.read(left.data(), static_cast<std::streamsize>(left.size()));
        second.read(right.data(), static_cast<std::streamsize>(right.size()));

        const auto readA = first.gcount();
        const auto readB = second.gcount();
        if (readA != readB) {
            return false;
        }
        ...
        if (!std::equal(left.begin(), left.begin() + readA, right.begin())) {
            return false;
        }
    }
...
fs::path BuildUniqueLibraryPath(const fs::path& directory, const fs::path& source) {
    ...
    fs::path candidate = directory / (stem + ext);
    ...
    return directory / (stem + L"_" + TimestampSuffix() + ext);
}
...
std::wstring ImportVideoToLibrary(const std::wstring& inputPath) {
    try {
        ...
        if (source.parent_path() == destinationDir) {
        ...
        if (!ec) {
            for (const auto& entry : fs::directory_iterator(destinationDir)) {
                if (!entry.is_regular_file()) {
                    continue;
                }
                const auto& target = entry.path();
                if (ToLower(target.extension().wstring()) != ToLower(source.extension().wstring())) {
                    continue;
                }
                const auto targetSize = fs::file_size(target, ec);
                if (ec || targetSize != sourceSize) {
                    ec.clear();
                    continue;
                }
                if (AreFilesIdentical(source, target)) {
                    SetStatus(L"Video already exists in library. Reusing saved copy.");
                    return target.wstring();
                }
            }
        }

        const fs::path destination = BuildUniqueLibraryPath(destinationDir, source);
...
bool RegisterWindowClasses(HINSTANCE instance) {
    ...
    controllerClass.hIcon = LoadIconW(instance, MAKEINTRESOURCEW(IDI_APP_ICON));
    controllerClass.hIconSm = LoadIconW(instance, MAKEINTRESOURCEW(IDI_APP_ICON));
    ...
    wallpaperClass.hIcon = LoadIconW(instance, MAKEINTRESOURCEW(IDI_APP_ICON));
    wallpaperClass.hIconSm = LoadIconW(instance, MAKEINTRESOURCEW(IDI_APP_ICON));
...
int WINAPI wWinMain(HINSTANCE instance, HINSTANCE, PWSTR, int showCommand) {
    ...
    if (background) {
        ...
        ShowWindow(g_app.controller, showCommand);
